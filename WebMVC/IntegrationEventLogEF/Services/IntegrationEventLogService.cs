...
namespace IntegrationEventLogEF.Services
{
    public class IntegrationEventLogService : IIntegrationEventLogService
    {
        ...
        public Task SaveEventAsync(List<IntegrationEvent> @event, DbTransaction transaction)
        {
            if (transaction == null)
            {
            ...
            foreach (IntegrationEvent pEvent in @event)
            {
                var eventLogEntry = new IntegrationEventLogEntry(pEvent);                
        ...
        public Task MarkEventAsProcessedAsync(IntegrationEvent @event)
        {
            var eventLogEntry = _integrationEventLogContext.IntegrationEventLogs.Single(ie => ie.EventId == @event.Id);
            eventLogEntry.TimesSent++;
            eventLogEntry.State = EventStateEnum.Processed;
